...
$query = "SELECT * FROM gastronomia WHERE pr_id=$pr_id AND ca_id=$ca_id AND cat_id=$cat_id AND si_id=$si_id";
                                        ...
                                        if (count($arrGastronomia) > 0) {
                                            ...
                                            for ($r = 0; $r < count($arrGastronomia); $r++) {
                                                ...
                                                for ($i = 0; $i < count($arrP); $i++) {
                                                    if ($arrP[$i]->pr_id == $arrGastronomia[$r]->pr_id) {
                                                        echo '' . $arrP[$i]->pr_nombre . ' | ';
                                                    }
                                                }
                                                for ($i = 0; $i < count($arrCan); $i++) {
                                                    if ($arrCan[$i]->ca_id == $arrGastronomia[$r]->ca_id) {
                                                        echo '' . $arrCan[$i]->ca_nombre . ' | ';
                                                    }
                                                }
                                                for ($i = 0; $i < count($arrCat); $i++) {
                                                    if ($arrCat[$i]->cat_id == $arrGastronomia[$r]->cat_id) {
                                                        echo $arrCat[$i]->cat_nombre . ' - ';
                                                    }
                                                }
                                                for ($i = 0; $i < count($arrSit); $i++) {
                                                    if ($arrSit[$i]->si_id == $arrGastronomia[$r]->si_id) {
                ...
                <div id="myCarousel" class="carousel slide">
                    ...
                    <div class="carousel-inner" align="center"> 
                        <div class="item active">
                            ...
                            echo "<img class='FotoView' src='../../controlador/gestion/verGastronomiaRpt.php?ga_id=" . $arrGastronomia[0]->ga_id . "'/>  ";
                            ?>
                            <div class="carousel-caption">
                                <h4><?php
                            echo $arrGastronomia[0]->go_nombre . '</br>';
                            ?> </h4>
                                <p>
                                    <?php
                                    echo $arrGastronomia[0]->ga_descripcion;
                                    ?>
                                </p>
                            </div>
                        </div>
                        <?php
                        for ($r = 1; $r < count($arrGastronomia); $r++) {
                            ?>
                            <div class="item">
                                <?php
                                echo "<td> <img class='FotoView' src='../../controlador/gestion/verGastronomiaRpt.php?ga_id=". $arrGastronomia[$r]->ga_id . "'/>  </td>";
                                ?> 
                                <div class="carousel-caption">
                                    <h4>
                                        <?php
                                        echo $arrGastronomia[$r]->ga_nombre . '</br>';
                                        ?> </h4>
                                    <p>
                                        <?php
